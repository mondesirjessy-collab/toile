...
describe('atelier workspace markup', () => {
  ...
  it('exposes the OBJ body import control (v185)', () => {
    ...
    expect(html).toContain('du brief au patron · v199');
  });
  ...
  it('exposes the robe archetype button (v194)', () => {
  ...
  it('exposes the doudoune archetype button (v199)', () => {
    expect(countId('at-doudoune')).toBe(1);
    expect(html).toMatch(/<button id="at-doudoune"[^>]*>..? Doudoune<\/button>/);
  });
