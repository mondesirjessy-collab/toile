 ...
 * per-constraint compliance (stretch vs bending) survives it.
 */

/** Build an edge, defaulting rest/compliance so fixtures stay terse. */
const e = (i: number, j: number, compliance = 0, rest = 1): Edge => ({ i, j, rest, compliance });
...
const edgeKey = (edge: Edge): string => {
  ...
  return `${a}-${b}:${edge.rest}:${edge.compliance}`;
};
...
describe('colorConstraints', () => {
  ...
  it('preserves every edge exactly once and keeps counts/offsets consistent', () => {
    // Two overlapping triangles with mixed stretch (0) and bending (1e-5)
    // compliance, so the preservation check also covers the compliance field.
    const edges = [
      e(0, 1, 0),
      e(1, 2, 0),
      e(2, 0, 0),
