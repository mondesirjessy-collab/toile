...
- ✅ **Perf reconstruction (v69)** : les ~12 pipelines GPU (9 solveur + 3 renderer) sont désormais compilés UNE FOIS par carte graphique (WeakMap par device) au lieu de recompilés à chaque build(). MESURÉ : glisser un curseur de mensuration (corps en cache) passe de ~1,0 s → ~80 ms (~12×). Restent au coût plein : le PREMIER build d'une mensuration jamais vue (surfaceNets du corps morphé ~1 s, coût v64) et le mesh-gen CPU des formes complexes (setin/chemise ~1,7 s) = fiche « generateSeamedPanels coûteux » encore ouverte.
