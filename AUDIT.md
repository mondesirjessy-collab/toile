...
- **Toutes les majeures ET le dernier mineur VISIBLE (M24) traités.**

## v78 — audit bloc mineur non visible : 16 correctifs sûrs + déferrals documentés (Opus, 08/07/2026)
Chaque fiche du bloc mineur re-vérifiée contre le code COURANT (workflow 27 fiches × 2 passes adversariales — beaucoup étaient « non contre-vérifiées » et les n° de ligne avaient dérivé). Corrigés (invisibles ou amélioration, aucun changement de drapé) :
- ✅ **M9(a)** : le sondage readPositions 300 ms ne tourne plus EN VEILLE (positions figées → posCache reste valide ; tout wake() le ré-arme) — batterie au repos. *M9(b) (repli du resolve timestamp dans l'encodeur de rendu) DÉFÉRÉ : gain ~50-100 µs/frame pour un risque sur le chemin de submit — faible ROI.*
- ✅ **M12** : `combineClothMeshes.seamCount += crossSeams.length` — les coutures inter-vêtements (robe froncée : 128 à n=64) étaient omises ; les totaux par-kind somment enfin à constraintCount. Test.
- ✅ **M14** : garde de connexité (flood-fill 4-voisins `countMaskIslands`, exporté+testé) — un décolleté trop échancré ou une station pincée qui SÉPARE le patron (fragments qui tombent) est signalé (console.warn « patron déconnecté ») au lieu d'échouer en silence ; couvre aussi la classe M10 (soudure setin). Warning-only → zéro changement de géométrie. 4 tests (n×n : setin=3 îlots, robe normale=1, sur-échancré warns).
- ✅ **M18** : borne de rejet + AABB des colliders conservatrices pour TOUT squash (× max(1,s)) — une future primitive s>1 ne peut plus sortir de la sphère de rejet et faire tunneliser en silence. No-op sur les corps actuels (tous s∈(0,1]).
- ✅ **M22 (reste)** : carré de contrôle 100 mm + flèche droit-fil + consignes déplacés sur une PAGE DE COUVERTURE dédiée (plus de chevauchement du patron page A1) ; rappel « marge 1 cm incluse » répété en marge de CHAQUE page.
- ✅ **M23** : export glTF — la pose (podium + bras) est échantillonnée DANS la boucle de retry, au même tick que la copie GPU gagnante, pas figée au clic → mannequin et vêtement sur la même frame.
- ✅ **M25** : export glTF assainit les positions/normales non finies (NaN/Inf → 0) — un état auto-collision explosé ne produit plus un .glb au min/max `null` rejeté par Blender.
- ✅ **M26** : import .toile.json = 1 SEUL rebuild au lieu de ~8-9 (drapeau `buildSuspended` + callbacks `onImportBegin/End`) — plus de gel multi-seconde ni de 8 re-chutes à res 128 ; état final identique.
- ✅ **M28** : le readback en vol capture l'identité du système (`const sys = system` + bail si `sys !== system`) — après un rebuild, une lecture de l'ANCIEN vêtement ne réécrit plus posCache/détecteur de veille (clic mal ciblé pendant ~300 ms après chaque changement).
- ✅ **M29** : `fitMap` (carte de tension) est un réglage qui SURVIT au rebuild (`renderer.setFitMap(fitMap)` dans build) — avant, toucher un curseur éteignait la carte alors que la case restait cochée.
- ✅ **M30 (reste)** : `motifCouleur` importé validé élément par élément (fini ∈ [0,1]) — le cœur (substeps/résolution/exposants) était déjà borné v75.
- ✅ **M33** : le HUD compte les particules SIMULÉES (invMass>0), pas la grille 2·n² pleine — une robe aline à n=64 affiche le vrai tissu, pas 8192 dont ~35 % morts.
- ✅ **M34** : les curseurs de patron robe ne TÉLÉPORTENT plus hors de 'tenue' (qui rend déjà cette robe) ; 'tenue' reçoit ses poignées de silhouette dans l'encart 2D.
- ✅ **M35** : « animation bras » sur un scan/drapé (animPrims null) ne bloque plus la veille pour rien (`!(animate && animPrims)`) ; la DÉCOCHER restaure la pose de repos EN PLACE (colliders + mesh) au lieu de `build()` qui re-larguait le vêtement posé.
- ✅ **M36** : la case « auto-collision » réveille le solveur (wake()) — avant, aucun effet visible sur une scène endormie.
- ✅ **M37** : `pickParticle` saute les particules immobiles (épinglées/tackées/découpées) via `system.isMovable()` → un clic dessus tourne la caméra au lieu d'un clic mort ; le double-clic reste non filtré (les épingles restent retirables).
- **DÉFÉRÉS avec justification (changements de physique ajustée ou résolutions inatteignables)** — voir commentaires dans le code :
  - ⏸️ **M2** (α dihedral fold-dependent) : le correctif exact (÷ (denom + α·(1−d²))) est mathématiquement juste MAIS raidit la flexion de façon non uniforme et casse les 7 presets calibrés dessus + les charnières de repassage → changement VISIBLE, exige un re-tuning couplé sous revue. Commentaire « KNOWN, INTENTIONALLY UNCHANGED » posé dans dihedral.wgsl pour éviter un « fix » naïf.
  - ⏸️ **M3** (exclusion miroir trans-panneau trop large) : RÉEL (contre-vérifié — non réfutable), mais ne mord QUE sur les tubes SANS corps (mirror-aligned front/back tunnelise) — la seule scène concernée est 'couture' (démo). Le correctif exige un buffer par-particule de distance-à-la-couture (BFS) + un NOUVEAU binding GPU sur le chemin chaud de l'auto-collision, et les cellules de couture diffèrent entre rect (couture) et formes découpées → mérite un chantier dédié à A/B, PAS un lot invisible (risque de casser l'auto-collision de TOUTES les scènes pour un gain de niche). Design complet dans la fiche détaillée.
  - ⏸️ **M4** (marge sd_body 2 gaps → couche ≥4) : latent — aucune scène ne dépasse la couche 1, et (réfuté) le rejet d'une primitive isolée est sûr jusqu'à la couche 7 (smin isolé = distance brute). Non touché (chemin de collision chaud pour un cas inatteignable).
  - ⏸️ **M6** (hash reconstruit chaque substep) : optimisation qui CHANGE la qualité de collision (hash périmé) → risque de régression pour un gain perf non prioritaire.
  - ⏸️ **M10** (soudure setin à n=18) : n=18 n'est pas sélectionnable (UI verrouillée 32/64/128) ; la classe est désormais couverte par la garde de connexité M14.
  - ⏸️ **M13** (rest de couture au spacing horizontal quelle que soit l'orientation) : raidit/relâche les coutures selon l'orientation = changement de drapé ajusté, risque moyen.
  - ⏸️ **M15** (bande elasticTop = 1 rangée à n≤17) : n≤17 non sélectionnable.
  - ⏸️ **M16** (bande épaule 0,79H rogne le deltoïde du scan) : scan seulement, ~4 % de sous-estimation, risque de changer la gradation.
- ℹ️ **M17** DÉJÀ corrigé (thighY suit déjà la morpho) ; **M27** = notes de vérification (rien à faire).
- **RESTE après v78** : uniquement les déférés ci-dessus (M2/M3/M4/M6/M13/M15/M16), tous physique-ajustée ou hors-UI — plus aucun correctif « sûr et invisible » en attente.

## v79 — les réglages de physique différés, traités avec A/B avant/après (Opus, 08/07/2026)
Reprise des 7 déférés physique de v78, chacun isolé avec comparaison avant/après sur les scènes concernées (109 tests) :
- ✅ **M4 (marge sd_body couche≥4)** : marge de rejet = `cloth_thickness + max(2, max_layer)·layer_gap + blend_k + 0.012` (uniforme `max_layer` @168, calculé au build). IDENTIQUE pour toute scène livrée (couche ≤ 1 → max(2,·)=2, exactement l'ancienne marge) ; ne grandit que pour un empilement 4+.
- ✅ **M13 (rest de couture par orientation)** : `seamRest = min(spacingU, spacingV)·0.15` (axe le plus serré) et cross-seam `min(a,b)·0.15`. Sous-mm sur des contraintes déjà quasi nulles ; robe froncée vérifiée cohérente, coutures intactes.
- ✅ **M15 (bande elasticTop)** : plancher à 2 rangées (`max(2, round(0.06·(n-1)))`) — identique à 32/64/128 (4/2/8 rangées), ne borne que n≤17 inatteignable.
- ✅ **M3 (exclusion miroir trans-panneau)** : IMPLÉMENTÉ. Champ par-particule `seamDist` (BFS en sauts de grille depuis les cellules cousues, clampé à 3), transporté comme seamFree, **binding 6 de selfCollide**. L'exclusion miroir front↔back ne s'applique QUE si les DEUX particules sont à ≤ 2 sauts d'une couture → la couture se ferme toujours, mais un contact miroir INTÉRIEUR (tube sans corps qui s'aplatit) est repoussé au lieu de tunneliser. QC : ensemble body-worn IDENTIQUE (10640/41546, coutures intactes — le corps sépare déjà les panneaux, l'exclusion intérieure ne se déclenchait jamais), couture cohérente. +2 tests (seamDist 0 aux coutures → 1 → 3, transport au merge). **PIÈGE HMR appris** : un shader `?raw` modifié ne recompile PAS le pipeline GPU en cache (WeakMap v69) — il FAUT redémarrer le serveur dev (kill+start), pas juste reload, sinon « binding 6 not present in layout ».
- ✅ **M2 (α dihedral fold-dependent)** : APPLIQUÉ. `s = -c·√fold / (denom + alpha·fold)` où `fold = max(1-d², 0)` (le terme alpha porte enfin le (1-d²) du numérateur → compliance_bend = même raideur à tout angle de pli). **Contre-vérifié empiriquement NON régressif** (c'était LA crainte de v78) : l'équilibre posé est dominé par la gravité et les zones quasi-plates (fold≈0) sont inchangées. A/B : robe+Soie avant/après = drapé liquide identique ; drapé Soie (plis fins liquides) vs Denim (peu de plis raides) = gamme de presets préservée ; robe froncée (charnières de repassage, même s) = propre, couture intacte. La prédiction « high regression » du workflow raisonnait sur le scalaire s instantané, pas sur l'équilibre convergé à 20 sous-pas.
- ⏸️ **M6 (hash chaque substep)** : GARDÉ DIFFÉRÉ — décision motivée. Reconstruire le hash moins souvent est un compromis perf qui DÉGRADE la fraîcheur de l'auto-collision (hash périmé) ; ce moteur a bataillé contre le verrouillage du mauvais côté, tourne déjà à 30 fps, et la correction > perf. Mauvais échange, non un oubli.
- ⏸️ **M16 (bande épaule 0,79H scan)** : GARDÉ DIFFÉRÉ — asymétrie de risque. Abaisser la bande pour capter le vrai deltoïde risque de capter le BRAS pendant (surestimation = pire que la sous-estimation ~4 % actuelle, scan seulement). Le plancher 0,79H est un choix conservateur qui marche.
- **BILAN AUDIT** : 3 critiques + toutes les majeures + TOUT le bloc mineur traité (corrigé OU différé avec justification écrite). Il ne reste que M6 et M16, deux compromis où NE PAS changer est le bon choix d'ingénierie (documenté dans le code et ici).

## v80-81 — AUDIT MOTEUR + PHYSIQUE : « les tissus sont rigides » + robustesse multi-machines (Opus, 08/07/2026)
Nouvel audit sur plainte utilisateur (tissus rigides) + objectif « physique au max, tourne partout ». Workflow 5 lentilles (souplesse, correction solveur, perf, robustesse WebGPU, plafond réalisme) → 38 findings contre-vérifiés + synthèse, + A/B live.
- **CAUSE RACINE de la raideur (confirmée en direct)** : la FLEXION était calibrée 10-100× trop raide. Preuve A/B : robe Jersey = carton lisse sans plis ; en gardant l'étirement RIGIDE (le vêtement tient) + flexion MOLLE → plis souples réalistes. Deux hypothèses de l'audit RÉFUTÉES par le code : PAS de mismatch GUI/sim (applyPreset('Jersey') synchronise les deux au démarrage), et M2 n'est PAS un bug (XPBD exact, ne pas reverter).
- ✅ **v80 souplesse** : (1) dihedral.wgsl plancher near-flat `foldc = max(fold, 0.04)` — quand d→±1, alpha·fold→0 et la projection perd toute autorité de compliance (le tissu résiste à son PREMIER pli à pleine raideur) ; le plancher plafonne le raidissement near-flat à ~25× → le premier pli se forme. (2) PRESETS recalibrés bend 5-10× plus mou + ORDRE monotone corrigé (les lins/laines drapants étaient PLUS raides que le jersey → « choisir un tissu fluide » raidissait) : Soie 5e-4, Jersey/Maille 2e-4, Laine 1.5e-4, Lin 8e-5, Popeline 3e-5, Denim 5e-6 ; shear desserré sur les tissés fluides. **Étirement laissé RIGIDE** (le mou le fait tomber du corps — vérifié). (3) damping 0.8→0.5. A/B : Jersey robe = plis souples (avant carton), Denim = structuré/raide (gamme préservée), ensemble tient, 30 fps.
- ✅ **v81 robustesse (tourne sur la majorité des machines)** : (1) **gouverneur de substeps adaptatif** — le slider = PLAFOND ; sur GPU faible le nb de substeps effectif descend (plancher 8) pour garder le framerate, piloté par le VRAI temps frame mur (pas le dt clampé, pas le timestamp GPU=0 sur ces machines). HUD « (auto) » quand il plafonne. Vérifié : reste à 20 sur machine rapide. (2) **fallback d'adaptateur** (Device.ts) : si `high-performance` = null (portables à GPU intégré), re-`requestAdapter()` sans option avant d'échouer. (3) **garde-NaN** integrate.wgsl : `if (any(x != x))` → gèle la particule à sa position précédente (GPU mobile fp relâché). (4) **clamp canvas à maxTextureDimension2D** (écrans 5K/6K dpr 2 → plus de canvas blanc). (5) **bouton « Recharger »** dans le banner fatal (device.lost/TDR laptop → chemin de récup). (6) **drag aéro quadratique** updateVelocity.wgsl (`-0.1·speed·dt`) : le mouvement rapide freine plus que le settle lent → le tissu flotte/flotte au vent au lieu d'une plaque rigide ; ∝ vitesse donc NUL au repos (drapé posé inchangé).
- **DÉFÉRÉS motivés (inchangés)** : auto-collision sur stride (M6 — le gouverneur couvre déjà la perf GPU faible, et strider risque la qualité de collision = la saga du verrouillage mauvais côté) ; buffer readback réutilisable (l'historique v45 = le buffer jetable corrige un vrai bug de poisoning) ; friction tissu-tissu (risque d'aggraver le « figé » tant que la souplesse se stabilise). Le reste de la synthèse (skip render au repos, uncapturederror par type) = backlog perf/robustesse basse priorité.
- Script workflow réutilisable : `engine-physics-audit-wf_f518f1de-f9e.js`. LEÇON : sur une plainte visuelle, reproduire+A/B en direct AVANT de conclure ; le workflow a bien trouvé la cause (bend trop raide) mais s'est trompé sur 2 sous-points (mismatch GUI, reverter M2) — corrigés en lisant le code.
...
### M3. [✅ borné v74] Self-collision tunneling at low substep counts, then repulsion-only wrong-side lock
...
### M3. [✅ corrigé v79 — seamDist BFS + binding 6, A/B body-worn identique] Cross-panel mirror exclusion applies across the entire panel interior — a flattened front/back tube interpenetrates at mirror-aligned points
**Fichier** : `src/engine/solver/shaders/selfCollide.wgsl`
...
### M21. [✅ corrigé v76] PDF tiling prints an extra near-empty page when the layout exceeds a page multiple by less than the OVERLAP band *(non contre-vérifié)*
...
### M22. [✅ corrigé v76 (clip cadre ; recouvrement carré-contrôle page A1 = layout mineur restant)] Cut lines are bbox-filtered but never clipped — segments overstrike the page label, glue guides, and page-1 instructions; calibration square sits on top of the pattern *(non contre-vérifié)*
**Fichier** : `/Users/jessymondesir/dev/toile/src/app/patternPdf.ts`
